...
<?php # Script 1.7 - add_task_sprintf.php
/**
 * This page adds tasks to the tasks table.
 * The page both displays and handles the form.
 * The code uses sprintf to generate the SQL query.
 * 
 */

// echo __DIR__;
// output: C:\Apache24\htdocs\advancedPHP\chap1

$config = parse_ini_file(__DIR__ . '/config.ini');
// print_r($config);

// connect to the database:
$dbc = mysqli_connect($config['hostname'], $config['username'], $config['password'], $config['schema']);
// echo FILTER_VALIDATE_INT;  => 257 ;  php internal const.
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['task'])) {
    ...
    // Escape the task:
    // we need the $dbc for charset info.
    // $task = mysqli_real_escape_string($dbc, strip_tags($_POST['task'])); 

    // Add the task to the database.
    // $q = "INSERT INTO tasks (parent_id, task, date_added, date_completed) VALUES ($parent_id, '$task', UTC_TIMESTAMP(), '0000-00-00 00:00:00')";

    $q = sprintf("INSERT INTO tasks (parent_id, task, date_added, date_completed) 
                  VALUES (%d, '%s', UTC_TIMESTAMP(), '0000-00-00 00:00:00')",
                 $parent_id, mysqli_real_escape_string($dbc, strip_tags($_POST['task'])));
...
// Display the form:
echo '<form action="add_task_sprintf.php" method="post">
<fieldset>
...
// Rechieve all the uncompleted tasks
